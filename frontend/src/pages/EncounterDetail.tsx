  ...
  return (
    <div className="card overflow-hidden border-l-4 border-ace-500">
      ...
      <div className="p-4 space-y-2 text-sm">
        ...
        {hcc.recapture_gaps?.length > 0 && (
          <div className="rounded-lg bg-amber-50 ring-1 ring-amber-200 p-2.5">
            <div className="text-amber-800 font-semibold text-sm">
              Recapture opportunity — documented last year, not yet this year
            </div>
            <div className="mt-1 space-y-0.5">
              {hcc.recapture_gaps.map((g) => (
                <div key={g.hcc} className="text-amber-700 text-sm">
                  HCC {g.hcc} · {g.label} <span className="font-semibold">+{g.coefficient.toFixed(3)} RAF</span>
                </div>
              ))}
            </div>
            <div className="mt-1 text-xs text-amber-600">
              {hcc.recapture_gaps.reduce((s, g) => s + g.coefficient, 0).toFixed(3)} RAF at stake —
              conditions must be re-documented each year to risk-adjust.
            </div>
          </div>
        )}
      </div>
